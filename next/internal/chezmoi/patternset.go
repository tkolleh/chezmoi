...
// Glob returns all matches in vos.
func (ps *PatternSet) Glob(vos doublestar.OS, prefix string) ([]string, error) {
	...
	for i, match := range matchesSlice {
		matchesSlice[i] = strings.TrimPrefix(filepath.ToSlash(match), prefix)
	}
